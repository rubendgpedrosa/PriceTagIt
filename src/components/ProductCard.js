...
const ProductCard = ({ product, deleteItem }) => {
  ...
  return (
    <div className="relative items-center max-w-sm mx-auto">
      ...
      {!confirmDelete?<button onClick={() => setConfirmDelete(!confirmDelete)} className="absolute right-0 top-0 text-red-500 rounded-full p-2 focus:outline-none">
        <svg className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
        </svg>
        </button>:
        <button onClick={() => deleteItem(product)} className="absolute right-0 top-0 text-red-500 rounded-full p-2 focus:outline-none">
        <svg className="h-5 w-5"  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
